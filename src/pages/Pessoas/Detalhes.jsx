...
export function Detalhes() {
  ...
  function PaxAceito() {
    ...
    set(ref(db, "CaronasAtivas/" + userUID + "/" + keyCarona), {
      email: params.item.email,
      image: params.item.image,
      data: params.item.data,
      destino: params.item.destino,
  ...
  function CheckData() {
    ...
    let output =
      ...
      hj.getFullYear();
    console.log(hrAgora);
    if (params.item.horario <= hrAgora && params.item.data === output) {
      console.log("Já começou");
      setIniciou(true)
    } else {
  ...
  useEffect(() => {
    ...
    async function ler() {
      ...
      let { request } = await requestBackgroundPermissionsAsync();
      if (status !== "granted") {
        Alert.alert(
          "Atenção...",
          "Permissão ao acesso a localização foi negado"
        );
        return;
      }

      let locationCurrent = await getCurrentPositionAsync();
      setLocation({
      ...
      <View style={styles.container}>
          ...
          <View style={styles.modalEditar}>
            ...
            >
              <TouchableOpacity
                ...
                onPress={() =>
                  updateCarona(vagas, horario, data, placa) ||
                  navigation.navigate("CaronasDisponiveis")
                }
              >
        ...
        >
          <View style={styles.modal4}>
            ...
            >
              <View
                style={{ flexDirection: "column", width: 150, height: 150 }}
              >
                <LottieView
                  source={require("../../Assets/104493-whatsapp-loop.json")}
                  autoPlay={true}
                  loop={true}
                  style={{
                    alignSelf: "center",
                ...
                <TouchableOpacity
                  ...
                  style={styles.botaoModal4}
                >
                  <Text style={styles.textBotao}>WHATSAPP</Text>
                </TouchableOpacity>
              </View>
              <View
                style={{ flexDirection: "column", width: 150, height: 200 }}
              >
                <LottieView
                  source={require("../../Assets/72126-email-verification.json")}
                  autoPlay={true}
                  loop={true}
                  style={{
                    alignSelf: "center",
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  <View style={{ position: "absolute", top: 25, left: 75 }}>
                    <Text
                      style={{ fontSize: 17, color:'#0e008a', textDecorationLine:'underline' }}
                      onPress={() => {
        ...
        </View>
        {
          iniciou && (
              <View>
                <Text style={{fontSize:25, marginBottom:10, borderWidth:1, textTransform:"uppercase", backgroundColor:'#F6C445'}}> Carona já iniciada</Text>
              </View>
          )
        ...
        {dono && (
          <TouchableOpacity
            style={styles.botaoVerInteressados}
